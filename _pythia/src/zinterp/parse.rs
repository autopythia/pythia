...
impl From<(Span, SafeStr)> for SpanStr {
  fn from(t: (Span, SafeStr)) -> SpanStr {
    SpanStr{span: t.0, str_: t.1}
  }
}

impl<'a> From<(Span, &'a SafeStr)> for SpanStr {
  fn from(t: (Span, &'a SafeStr)) -> SpanStr {
    SpanStr{span: t.0, str_: t.1.clone()}
  }
}

impl AsRef<SafeStr> for SpanStr {
  fn as_ref(&self) -> &SafeStr {
    &self.str_
  }
}

impl Display for SpanStr {
...
pub type SpanIdent = SpanStr;
pub type Lit = SafeStr;
pub type TermRef = Rc<Term>;
pub type SentRef = Rc<Sent>;

#[derive(Debug)]
pub enum Term {
  // TODO TODO
  Ident(Span, SpanIdent),
  // NB: deprecated qual ident syntax.
  ...
  SetLit(Span, Vec<TermRef>),
  DictLit(Span, Vec<TermRef>),
  Neg(Span, TermRef),
  //Tuple(Vec<TermRef>),
  Group(Span, TermRef),
  Bunch(Span, Vec<TermRef>),
  // TODO: deprecated query operator?
  Query(Span, TermRef),
  PQuery(Span, TermRef),
  Attr(Span, TermRef, SpanIdent),
  BindAttr(Span, TermRef, SpanIdent, TermRef),
  RebindAttr(Span, TermRef, SpanIdent, TermRef),
  ...
  // TODO: below, non-grounded static terms (e.g. for enumeration).
  _StaticGlobal(i32),
  _StaticLocal(i32),
}
...
impl Term {
  pub fn span(&self) -> Span {
    match self {
      ...
      &Term::Bunch(ref span, ..) |
      &Term::Query(ref span, ..) |
      &Term::PQuery(ref span, ..) |
      &Term::Attr(ref span, ..) |
      &Term::BindAttr(ref span, ..) |
      &Term::RebindAttr(ref span, ..) |
      &Term::Equal(ref span, ..) |
...
pub enum Sent {
  ...
  Pass(Span),
  Raise(Span, TermRef),
  Global(Span, SpanIdent),
  Nonlocal(Span, Option<i16>, SpanIdent),
  With(Span, TermRef, Vec<SentRef>),
  Try,
  If_(Span, Vec<(TermRef, Vec<SentRef>)>, Option<Vec<SentRef>>),
  If(Span, TermRef, Vec<SentRef>),
  Elif(Span, TermRef, Vec<SentRef>),
  Else(Span, Vec<SentRef>),
  While,
  For,
  Match(Span, (), Vec<SentRef>),
  Def(Span, Option<DefPrefix>, (), Vec<SentRef>),
  // FIXME: def-like args (params) are more general than just
  // a list of idents; but this is a stopgap to parse something.
  Defproc(Span, Option<DefPrefix>, SpanIdent, Vec<Option<SpanIdent>>, Vec<SentRef>),
  Defmatch(Span, Option<DefPrefix>, SpanIdent, Vec<Option<SpanIdent>>, Vec<SentRef>),
  //Enum(Span, (), ),
  Cases(Span, (), Vec<SentRef>),
  Class(Span, (), Vec<SentRef>),
  Quote(Span, (), Vec<SentRef>),
  _EndQuote(Span),
}
...
impl SentPrefix {
  pub fn into_def(self) -> Option<DefPrefix> {
    match self {
      SentPrefix::_Nil => None,
      //SentPrefix::Global => Some(DefPrefix::Global),
      SentPrefix::Rule => Some(DefPrefix::Rule),
    }
  }
}
...
impl<'a> From<&'a Token> for SentStage {
  fn from(tok: &'a Token) -> SentStage {
    match tok {
      &Token::Elif => SentStage::Elif,
      &Token::Else => SentStage::Else,
      &Token::Except => SentStage::Except,
      &Token::Finally => SentStage::Finally,
      &Token::If => SentStage::If,
      &Token::Try => SentStage::Try,
      _ => panic!("bug")
    }
  }
}
...
impl SentIndent {
  pub fn is_eq(&self) -> bool {
    match self {
      &SentIndent::Eq(_) => true,
      &SentIndent::Gt(_) => false,
    }
  }

  pub fn is_gt(&self) -> bool {
    match self {
      &SentIndent::Eq(_) => false,
      &SentIndent::Gt(_) => true,
    }
  }

  pub fn eq_to_gt(&self) -> SentIndent {
    match self {
      &SentIndent::Eq(indent) => {
        SentIndent::Gt(indent)
      }
      _ => panic!("bug")
    }
  }
...
#[derive(Clone, Copy, Debug)]
pub struct SentCtx {
  _stage: Option<SentStage>,
  depth:  u16,
  indent: SentIndent,
  //bp: RawBp,
}

impl Default for SentCtx {
  fn default() -> SentCtx {
    SentCtx{
      _stage: None,
      depth:  0,
      indent: SentIndent::Eq(0),
      //bp: 0,
    }
  }
}
...
impl From<(Span, IoError)> for ParseSpanError {
  #[track_caller]
  fn from(t: (Span, IoError)) -> ParseSpanError {
    let loc = loc();
    ParseSpanError{span: t.0, err: ParseError::Io(t.1.to_string().into()), loc}
  }
}

/*pub trait ParseSpanErrorExt<Ret> {
...
pub struct ParserTAPState {
  buf:  Option<Box<dyn Write>>,
}

impl ParserTAPState {
  pub fn init() -> ParserTAPState {
    ParserTAPState{
      buf:  None,
    }
  }
...
impl<S: AsRef<str>> Parser<S> {
  pub fn new(buf: S) -> Parser<S> {
    ...
    //let verbose = 0;
    let tap = DebugOutput::default();
    let tap_ = ParserTAPState::init();
    Parser{tokens, cur, peek, tap, tap_}
  }

  pub fn set_tap_buffer(&mut self, buf: Box<dyn Write>) -> Option<Box<dyn Write>> {
    let prev_buf = self.tap_.buf.take();
    self.tap_.buf = Some(buf);
    prev_buf
  }
  ...
  pub fn maybe_cur(&self) -> Option<SpanToken> {
    self.cur.clone()
  }
  ...
  pub fn maybe_peek(&mut self) -> Option<SpanToken> {
    if self.cur.is_none() {
      return None;
    }
    Some(self.peek())
  ...
  pub fn mod_(&mut self) -> Result<Mod, ParseSpanError> {
    ...
    let depth = 0;
    let start = self.maybe_cur_span().map(|span| span.start).unwrap_or(0);
    if self.tap_.on() {
      self.tap_._write_indent(depth).map_err(|e| (Span{start, end: start}, e))?;
      {
        let buf = self.tap_.buf.as_mut().unwrap();
        writeln!(buf, "Mod").map_err(|e| (Span{start, end: start}, e))?;
      }
    }
    let mut body = Vec::new();
    loop {
      _debugln!(self, "DEBUG: Parser::mod_: stm...");
      match self.sent(SentCtx::default())? {
        None => break,
        Some((stm, _)) => {
          body.push(Rc::new(stm));
        }
      }
    }
    //let start = 0;
  ...
  pub fn term(&mut self, ctx: TermCtx) -> Result<Term, ParseSpanError> {
    ...
    self.next();
    let mut cur = self.cur();
    _debugln!(self, "DEBUG: Parser::term: depth={} cur span={:?} tok={:?}", ctx.depth, cur.span, &cur.tok);
    match &cur.tok {
      &Token::Indent(0) => {
        self.next();
        cur = self.cur();
    ...
    let mut lterm = self.term_nud(ctx.depth + 1, ctx.indent, cur.clone())?;
    ...
    loop {
      ...
      cur = next;
      lterm = self.term_led(ctx.depth + 1, ctx.indent, lterm, cur.clone())?;
      _debugln!(self, "DEBUG: Parser::term: led: tok={:?} lterm={:?}", &cur.tok, &lterm);
    }
    _debugln!(self, "DEBUG: Parser::term: depth={} done", ctx.depth);
    Ok(lterm)
  }
  ...
  pub fn _sent(&mut self, ctx: SentCtx, prefix: SentPrefix) -> Result<Option<(Sent, SentCtx)>, ParseSpanError> {
          ...
          match this_ctx.indent {
            SentIndent::Gt(this_indent) => {
              if indent <= this_indent {
                ...
                this_ctx.indent = SentIndent::Eq(indent);
              }
            }
            SentIndent::Eq(this_indent) => {
              if indent != this_indent {
                /*self.next();
                let peek = self.cur();
                self.restore(&peek.span);*/
    ...
    match &cur.tok {
      &Token::Global => {
        ...
        return Ok(Some((Sent::Global(span, ident.into()), this_ctx)));
        /*
    ...
    match &cur.tok {
      ...
      &Token::Try => {
        ...
        let mut this_ctx = SentCtx{
          _stage: Some(SentStage::from(&cur.tok)),
          depth:  this_ctx.depth + 1,
          indent: this_ctx.indent.eq_to_gt(),
        };
        let _stm = match self.sent(this_ctx)? {
          None => {
    ...
    match &cur.tok {
      ...
      &Token::DotIdent(_) => {
        ...
        let span = cur.span.hull(self.pos());
        return Ok(Some((Sent::Just(span, t.into()), this_ctx)));
      }
      &Token::TTTick => {
        _debugln!(self, "DEBUG: Parser::stm: end block quote: tok={:?}", &cur.tok);
        self.tokens.flag |= TokenizerFlag_::PYTHIA;
        return Ok(Some((Sent::_EndQuote(cur.span.clone()), this_ctx)));
      }
      &Token::Pass => {
        _debugln!(self, "DEBUG: Parser::stm: pass: tok={:?}", &cur.tok);
        return Ok(Some((Sent::Pass(cur.span.clone()), this_ctx)));
      }
      &Token::Raise => {
        _debugln!(self, "DEBUG: Parser::stm: raise: tok={:?}", &cur.tok);
        let start = cur.span.clone();
        // FIXME: spaces are required here.
        self.maybe_spaces_deprecated();
        let exc = self.term(this_ctx.term())?;
        let span = start.hull(self.pos());
        return Ok(Some((Sent::Raise(start, exc.into()), this_ctx)));
      }
      &Token::If => {
        ...
        // TODO
        let mut inner_ctx = SentCtx{
          _stage: None,
          depth:  this_ctx.depth + 1,
          indent: this_ctx.indent.eq_to_gt(),
        };
        let mut body = Vec::new();
        let stm = match self.sent(inner_ctx)? {
          None => {
            return Err((self.cur_span(), ParseError::ExpectedSent).into());
          }
          Some((stm, stm_ctx)) => {
            assert!(stm_ctx.indent.is_eq());
        ...
        let span = start.hull(self.pos());
        return Ok(Some((Sent::If(span, cond.into(), body), this_ctx)));
      }
      &Token::Elif => {
        let org_tok = cur.tok.clone();
        _debugln!(self, "DEBUG: Parser::stm: elif: tok={:?}", &cur.tok);
        let start = cur.span.clone();
        // FIXME: spaces are required here.
        self.maybe_spaces_deprecated();
        let cond = self.term(this_ctx.term())?;
        ...
        let span = start.hull(self.pos());
        return Ok(Some((Sent::Elif(span, cond.into(), body), this_ctx)));
      }
      &Token::Else => {
        let org_tok = cur.tok.clone();
        _debugln!(self, "DEBUG: Parser::stm: else: tok={:?}", &cur.tok);
        let start = cur.span.clone();
        // NB: optional spaces here.
        self.maybe_spaces_deprecated();
        self.next();
        let cur = self.cur();
        match &cur.tok {
        ...
        let span = start.hull(self.pos());
        return Ok(Some((Sent::Else(span, body), this_ctx)));
      }
      &Token::With => {
        ...
        return Ok(Some((Sent::With(span, head.into(), body), this_ctx)));
      }
      &Token::Defmatch |
      &Token::Defproc |
      &Token::Def => {
        ...
        loop {
          ...
          match &stm {
            &Sent::_EndQuote(_) => {
              ...
              self.tokens.flag |= TokenizerFlag_::PYTHIA;
              return Ok(Some((Sent::Quote(span, (), body), this_ctx)));
            }
            _ => {}
          }
          body.push(stm.into());
  ...
  pub fn term_nud(&mut self, ctx_depth: u16, ctx_indent: RawIndent, cur: SpanToken) -> Result<Term, ParseSpanError> {
    // TODO TODO
    ...
    _debugln!(self, "DEBUG: Parser::term_nud: cur span={:?} tok={:?}", cur.span, &cur.tok);
    match &cur.tok {
      &Token::Indent(0) => {
      ...
      &Token::PlaceIdent(ref s) => {
        ...
        return Ok(Term::Ident(cur.span.clone(), (cur.span, s).into()));
      ...
      &Token::LParen => {
        ...
        let rterm = self.term(this_ctx)?;
        self.maybe_term_spaces(ctx_indent)?;
        _debugln!(self, "DEBUG: Parser::term_nud: LParen: 0 span={:?} tok={:?}", cur.span, &cur.tok);
        self.next();
        let cur = self.cur();
        _debugln!(self, "DEBUG: Parser::term_nud: LParen: 1 span={:?} tok={:?}", cur.span, &cur.tok);
        match &cur.tok {
          &Token::Comma => {
            // FIXME: HACK.
            self.next();
            ...
            match &cur.tok {
              &Token::RParen => {
                let span = start.hull(self.pos());
                return Ok(Term::TupleLit(span, vec![rterm.into()]));
              }
              _ => {
  ...
  pub fn term_led(&mut self, ctx_depth: u16, ctx_indent: RawIndent, mut lterm: Term, cur: SpanToken) -> Result<Term, ParseSpanError> {
    let this_ctx = TermCtx{
      depth:  ctx_depth,
      indent: ctx_indent,
      bp: self.lbp(&cur.tok),
    };
    _debugln!(self, "DEBUG: Parser::term_led: cur span={:?} tok={:?}", cur.span, &cur.tok);
    match &cur.tok {
      &Token::Indent(_) |
      &Token::Space |
      &Token::NL |
      ...
      &Token::PlusQuery => {
        ...
        // an apply-query term.
        let span = lterm.span();
        return Ok(Term::PQuery(span, lterm.into()));
      }
      &Token::Dot => {
        let start = lterm.span();
        self.maybe_term_spaces(ctx_indent)?;
        let id = match self.term(this_ctx)? {
          Term::Ident(_, s) => {
            s
          }
          _ => {
        ...
        match peek.as_ref().map(|t| &t.tok) {
          Some(&Token::Equal) => {
            ...
            let rterm = self.term(this_ctx)?;
            let span = start.hull(self.pos());
            return Ok(Term::BindAttr(span, lterm.into(), id.into(), rterm.into()));
          }
          Some(&Token::LWalrus) => {
            // TODO: next?
            self.maybe_term_spaces(ctx_indent)?;
            let rterm = self.term(this_ctx)?;
            let span = start.hull(self.pos());
            return Ok(Term::RebindAttr(span, lterm.into(), id.into(), rterm.into()));
          }
          _ => {
            let span = start.hull(self.pos());
            return Ok(Term::Attr(span, lterm.into(), id.into()));
          }
        }
      }
      &Token::Equal => {
      ...
      &Token::Comma => {
        ...
        match peek.as_ref().map(|t| &t.tok) {
          ...
          Some(&Token::RParen) => {
            match &lterm {
              &Term::Bunch(ref span, ref tup) => {
                return Ok(lterm);
              }
              _ => {}
            }
            let span = start.hull(self.pos());
            return Ok(Term::Bunch(span, vec![lterm.into()]));
          }
          _ => {}
        }
        let rterm = self.term(this_ctx)?;
        ...
        loop {
          ...
          match &cur.tok {
            &Token::RParen => {
              let span = start.hull(self.pos());
              if tup.len() == 2 {
                let t = tup.pop().unwrap();
                match &*t {
                  &Term::Bunch(_, ref tup_) => {
                    tup.extend(tup_.clone());
...
impl<S: AsRef<str>> Printer<S> {
  ...
  pub fn _pretty_print_stm(&self, stm: &Sent, level: RawIndent) {
    ...
    match stm {
      ...
      &Sent::Defmatch(_, prefix, ref head, ref params, ref body) => {
        match prefix {
          None => {}
          Some(DefPrefix::Rule) => {
            print!("rule ");
      ...
      &Sent::Defproc(_, prefix, ref head, ref params, ref body) => {
        match prefix {
          None => {}
          Some(DefPrefix::Rule) => {
            print!("rule ");
  ...
  pub fn _pretty_print_term(&self, term: &Term, level: RawIndent) {
    match term {
      ...
      &Term::PQuery(_, ref term) => {
        self._pretty_print_term(term, level);
        print!("?");
      }
      &Term::Attr(_, ref lterm, ref rterm) => {
        self._pretty_print_term(lterm, level);
        print!("._");
        //print!(".{}", self._snippet(rterm.span));
      }
