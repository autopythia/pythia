...
const ZINTERP_MACH_TESTS: &'static str = include_str!("../../data/test/zinterp_mach.txt");
...
impl TestDataConfig {
  ...
  pub fn zinterp_mach_tests() -> TestDataConfig {
    let mut keys = Vec::new();
    for line in ZINTERP_MACH_TESTS.lines() {
      keys.push(line.into());
    }
    TestDataConfig{
      root: test_data_root(),
  ...
  pub fn zinterp_mach_test_1() -> TestDataConfig {
    let mut keys = Vec::new();
    for line in ZINTERP_MACH_TESTS.lines() {
      keys.push(line.into());
      break;
